...
for col in (GENES + CELLS):
  ...
  #  transformer = QuantileTransformer(n_quantiles=100,random_state=0, output_distribution="normal")
    ...
    if IS_TRAIN:
        ...
        transformer = pd.read_pickle(f'../input/simple-nn-old-cv-10folds-train/{col}_quantile_transformer.pkl')        
...
if IS_TRAIN:
    ...
    fa = pd.read_pickle(f'../input/simple-nn-old-cv-10folds-train/factor_analysis_g.pkl')
...
if IS_TRAIN:
    ...
    fa = pd.read_pickle(f'../input/simple-nn-old-cv-10folds-train/factor_analysis_c.pkl')
    #umap = pd.read_pickle(f'{MODEL_DIR}/{NB}_umap_g.pkl')
...
if IS_TRAIN:
    ...
    transformer = pd.read_pickle(f'../input/simple-nn-old-cv-10folds-train/{col}_quantile_transformer2.pkl')  
...
def fe_cluster_genes(train, test, n_clusters_g = 45, SEED = 123):
    ...
    def create_cluster(train, test, features, kind = 'g', n_clusters = n_clusters_g):
        ...
        kmeans_genes = load(open('../input/simple-nn-old-cv-10folds-train/kmeans_genes.pkl', 'rb'))
...
def fe_cluster_cells(train, test, n_clusters_c = 15, SEED = 123):
    ...
    def create_cluster(train, test, features, kind = 'c', n_clusters = n_clusters_c):
        train_ = train[features].copy()
        test_ = test[features].copy()
        kmeans_cells = load(open('../input/simple-nn-old-cv-10folds-train/kmeans_cells.pkl', 'rb'))
...
def run_training(fold, seed):
    ...
    model.load_state_dict(torch.load(f"../input/simple-nn-old-cv-10folds-train/SEED{seed}_FOLD{fold}_.pth"))
    model.to(DEVICE)
...
# np.save('oof_NN_oldcv_10fold.npy',y_pred)
...
sub.to_csv('submission_script_simpleNN_oldcv_10folds.csv', index=False)
