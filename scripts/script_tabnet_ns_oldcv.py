...
if IS_TRAIN:
    pca = PCA(n_components=n_comp, random_state=DEFAULT_SEED).fit(
        train_features[GENES])
    umap = UMAP(n_components=n_dim, random_state=DEFAULT_SEED).fit(
        train_features[GENES])
...
if IS_TRAIN:
    var_thresh = VarianceThreshold(
        threshold=0.5).fit(train_features.iloc[:, 4:])
...
train = train_features[train_features['cp_type']
...
def run_k_fold(train, test, feature_cols, target_cols, NFOLDS, seed):
    ...
    mskf = RepeatedMultilabelStratifiedKFold(
        n_splits=NFOLDS, n_repeats=NREPEATS, random_state=None)

    for f, (t_idx, v_idx) in enumerate(mskf.split(X=train, y=target)):
        oof_, pred_ = run_training(
            train, test, t_idx, v_idx, feature_cols, target_cols, f, seed)
...
def run_seeds(train, test, feature_cols, target_cols, nfolds=NFOLDS, nseed=NSEEDS):
    ...
    for seed in seed_list:
        ...
        oof_, predictions_ = run_k_fold(
            train, test, feature_cols, target_cols, nfolds, seed)
        oof += oof_ / nseed
...
train_trainbook = pd.read_pickle(
    "../input/503-203-tabnet-with-nonscored-features-train/interim/101_train_preprocessed.pkl")
test_trainbook = pd.read_pickle(
    "../input/503-203-tabnet-with-nonscored-features-train/interim/101_test_preprocessed.pkl")
...
feature_cols = [c for c in process_data(
    train).columns if c not in target_cols and c not in ['kfold', 'sig_id']]
...
train = train.merge(train_nonscored_pred[
                    train_targets_nonscored.columns], on='sig_id')
test = test.merge(test_nonscored_pred[
                  train_targets_nonscored.columns], on='sig_id')
...
nonscored_target = [
    c for c in train_targets_nonscored.columns if c != "sig_id"]
...
class LabelSmoothing(nn.Module):
    ...
    def forward(self, x, target):
        ...
        # print(target1.cpu())
        loss = -(torch.log(probs+1e-15) * target1 +
                 (1-target1) * torch.log(1-probs+1e-15))
        # print(loss.cpu())
...
def process_data(data):
    #     data = pd.get_dummies(data, columns=['cp_time','cp_dose'])
    data.loc[:, 'cp_time'] = data.loc[:, 'cp_time'].map(
        {24: 0, 48: 1, 72: 2, 0: 0, 1: 1, 2: 2})
    data.loc[:, 'cp_dose'] = data.loc[:, 'cp_dose'].map(
        {'D1': 0, 'D2': 1, 0: 0, 1: 1})
    return data
...
def run_training_tabnet(train, test, trn_idx, val_idx, feature_cols, target_cols, fold, seed, filename="tabnet"):
    ...
    model = TabNetRegressor(n_d=32, n_a=32, n_steps=1, lambda_sparse=0,
                            ...
                            optimizer_fn=torch.optim.Adam, optimizer_params=dict(
                                lr=2e-2, weight_decay=1e-5),
                            mask_type='entmax',  # device_name=DEVICE,
                            scheduler_params=dict(
                                milestones=[100, 150], gamma=0.9),  # )
...
#     eval_metric_nosmoothing = SmoothedLogLossMetric(0.)
    ...
    if IS_TRAIN:
        ...
        model.fit(X_train=x_train, y_train=y_train,
                  eval_set=[(x_valid, y_valid)], eval_metric=[
    ...
    valid_preds = torch.sigmoid(torch.as_tensor(
        valid_preds)).detach().cpu().numpy()
    oof[val_idx] = valid_preds

    predictions = model.predict(x_test)
    predictions = torch.sigmoid(torch.as_tensor(
...
def run_k_fold(train, test, feature_cols, target_cols, NFOLDS, seed):
    ...
    for f, (t_idx, v_idx) in enumerate(mskf.split(X=train, y=target)):
        oof_, pred_ = run_training_tabnet(
            train, test, t_idx, v_idx, feature_cols, target_cols, f, seed)
